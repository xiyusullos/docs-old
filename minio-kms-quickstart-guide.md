# KMS指南 [![Slack](https://slack.min.io/slack?type=svg)](https://slack.min.io)
...
T此步骤是可选的。但是，我们建议您通过TLS来上传/下载S3对象-尤其是当它们应该在
存储后端使用KMS 加密时。
...
由于我们希望稍后将一个/多个KES服务器连接到Vault，因此必须启用
AppRole身份验证。为此，请运行：
```sh
...
与Vault类似，KES `mlock`在Linux系统上使用syscall来防止OS将内存中的
